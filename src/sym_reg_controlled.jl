...
figsave_path = "$(fig_path)/$(dataset_name)_test_NN_$(nn_width)/sr_outputs_$(Dates.format(now(), "yyyy-mm-dd_HH-MM-SS"))/figs"
modelssave_path = "$(models_path)/$(dataset_name)_test_NN_$(nn_width)/sr_outputs_$(Dates.format(now(), "yyyy-mm-dd_HH-MM-SS"))"

mkpath(figsave_path)
mkpath(modelssave_path)

# Explicit SR domain
TMAX_SR_H = 960.0 # 240.0
...
t_hours_grid = unique(vcat(
    ...
    collect(241.0:15.0:960.0)   # 48
))

# β_grid       = collect(0.1:0.1:1.0)
...
# ============================================================
# 3b) BUILD SYNTHETIC VALIDATION DATASET
# ============================================================
t_h_val = Float64[]
t_norm_val = Float64[]
beta_val = Float64[]
...
y = inv(((β + 0.032583) * inv(t_norm * t_norm)) + 0.12298) * 0.033535
