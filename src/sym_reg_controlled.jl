...
figsave_path = "$(fig_path)/$(dataset_name)_test_NN_$(best_idx)$(use_multistart ? "_ms_test" : "")/sr_outputs_extended_recovered_$(Dates.format(now(), "yyyy-mm-dd_HH-MM-SS"))/figs"
modelssave_path = "$(models_path)/$(dataset_name)_test_NN_$(best_idx)$(use_multistart ? "_ms_test" : "")/sr_outputs_extended_recovered_$(Dates.format(now(), "yyyy-mm-dd_HH-MM-SS"))"
...
t_hours_val_grid = unique(vcat(
    ...
    collect(1530.0:20.0:TMAX_SR_H)
))
β_val_grid = collect(exp.(range(log(0.035), log(0.98), length=8)))
...
# Emphasize regions where teacher changes quickly (data-driven, no shape assumption)
# weights_syn = ones(Float64, length(y_syn))
# nT = length(t_hours_grid)
# dt = diff(t_hours_grid)

# for iβ in eachindex(β_grid)
#     idx = ((iβ - 1) * nT + 1):(iβ * nT)
#     yb = y_syn[idx]

#     slope = Vector{Float64}(undef, nT)
#     slope[1] = abs((yb[2] - yb[1]) / (dt[1] + eps(Float64)))
#     slope[end] = abs((yb[end] - yb[end - 1]) / (dt[end] + eps(Float64)))
#     for k in 2:(nT - 1)
#         slope[k] = abs((yb[k + 1] - yb[k - 1]) / (t_hours_grid[k + 1] - t_hours_grid[k - 1] + eps(Float64)))
#     end

#     slope ./= (maximum(slope) + eps(Float64))
#     weights_syn[idx] .= 1.0 .+ 4.0 .* slope
# end

# weights_syn ./= mean(weights_syn)
...
# ============================================================
# 3b) OPTIONAL VALIDATION DATASET
# ============================================================
use_validation = false

@info "SR train points: $(size(X_syn, 2))"
...
if use_validation
    ...
    for βi in β_val_grid
        for th in t_hours_val_grid
            push!(t_h_val, th)
            push!(t_norm_val, th / T_SCALE)
            push!(beta_val, βi)
        end
    end

    X_val = [t_norm_val'; beta_val']
    @assert size(X_val) == (2, length(t_norm_val))

    y_val = Vector{Float64}(undef, length(t_norm_val))
    for j in eachindex(t_norm_val)
        y_val[j] = chain((@view X_val[:, j]), best_nn)[1]
    end

    @info "SR validation points: $(size(X_val, 2))"
else
    X_val = X_syn
    y_val = y_syn
    @info "Validation disabled: using training grid for model selection"
end
...
# ============================================================
# 5) SYMBOLIC REGRESSION DIRECTLY ON y_NN
# ============================================================
# opts_warm = Options(
#     binary_operators = (+, *),
#     unary_operators  = (inv,),
...
# ============================================================
# 6) TAKE THE LAST FORMULA OF THE FRONTIER
# ============================================================
...
for member in frontier
    y_hat_val = sr_eval(member.tree, X_val)
    push!(val_losses, mean((y_val .- y_hat_val) .^ 2))
    push!(complexities, compute_complexity(member, opts))
end

# best_loss = minimum(val_losses)

# formule abbastanza buone: loss <= 1.5 * best_loss
# candidate_idx = findall(val_losses .<= 1.5 .* best_loss)

# Pick the most faithful formula on validation set
best_idx_front = argmin(val_losses)

# Tie-break: among near-identical losses, prefer simpler equation
tol = 1.02
...
#     best_idx_front = candidate_idx[1]
# else
#     candidate_idx = sort(candidate_idx, by = i -> complexities[i])

#     scores = fill(-Inf, length(candidate_idx))

#     for k in 2:length(candidate_idx)
#         i_prev = candidate_idx[k-1]
#         i_curr = candidate_idx[k]

#         Δc = complexities[i_curr] - complexities[i_prev]
#         if Δc > 0
#             scores[k] = (log(val_losses[i_prev]) - log(val_losses[i_curr])) / Δc
#         end
#     end

#     best_local = argmax(scores)
#     best_idx_front = candidate_idx[best_local]
# end
...
#     Plots.plot!(p_cmp, t_hours_grid, y_nn, label="NN β=$(round(βi, digits=2))")
#     Plots.plot!(p_cmp, t_hours_grid, y_sr, linestyle=:dash, label="SR β=$(round(βi, digits=2))")
# end
...
#     xlabel = "Time (h)",
#     ylabel = "rupture f(t_norm, β)",
#     title  = "SR surrogate (direct fit on y_NN)",
#     linewidth = 2,
#     legend = :best
# )
...
#     Plots.plot!(p_surr, t_hours_grid, y_sr, linestyle=:dash, label="SR β=$(round(βi, digits=2))")
# end
