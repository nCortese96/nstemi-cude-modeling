...
"""
    save_validation_patient_ids!(dataset_name, patients, report_dir; suffix="val")

Write an ordered ID CSV for a validation/test split generated by step 00.
"""
# Used by: scripts/00_run_preprocessing.jl.
function save_validation_patient_ids!(
    dataset_name::AbstractString,
    patients::AbstractVector{PatientData},
    report_dir::AbstractString;
    suffix::AbstractString="val",
)
    path = joinpath(report_dir, "ids_all_eligible_$(dataset_name)_$(suffix).csv")
    CSV.write(path, DataFrame(patient=[patient.id for patient in patients]))
    return path
end

"""
    select_gold_standard_patients(patients; dataset_tag, filters)

Apply the strict high-information filters used by the systematic truncation
workflow and return tagged gold-standard patients plus removal counts.
"""
# Used by: src/preprocessing.jl (save_gold_standard_artifacts!).
function select_gold_standard_patients(
    patients::AbstractVector{PatientData};
    dataset_tag::AbstractString,
    filters,
)
    anoms = find_anomalies(
        Vector{PatientData}(patients),
        filters.meas_min_number,
        filters.min_acq_time_before,
        filters.min_acq_n_before,
        filters.min_acq_time_after,
        filters.min_acq_n_after,
        filters.min_time;
        max_gap_h=filters.max_gap,
        verbose=false,
    )

    gold = filter(patient -> !haskey(anoms, patient.id), patients)
    tagged = [
        PatientData("$(dataset_tag)_$(patient.id)", copy(patient.timepoints), copy(patient.ctnt_data))
        for patient in gold
    ]

    return (patients=tagged, removed_anoms=length(anoms), gold_n=length(tagged))
end

"""
    save_gold_standard_artifacts!(entries, output_dir; run_dataset_name, filters)

Write canonical gold-standard IDs and filter-report artifacts for downstream
comparison workflows.
"""
# Used by: scripts/00_run_preprocessing.jl.
function save_gold_standard_artifacts!(
    entries,
    output_dir::AbstractString;
    run_dataset_name::AbstractString,
    filters,
)
    gold_patients = PatientData[]
    gold_dataset = String[]
    ...
    for entry in entries
        selected = select_gold_standard_patients(entry.patients; dataset_tag=entry.dataset_tag, filters=filters)
        append!(gold_patients, selected.patients)
        append!(gold_dataset, fill(entry.dataset_name, length(selected.patients)))

        push!(report, (
            dataset=entry.dataset_name,
            raw_n=entry.raw_n,
            eligible_n=length(entry.patients),
            removed_anoms=selected.removed_anoms,
            gold_n=selected.gold_n,
        ))
    end

    ids_path = joinpath(output_dir, "ids_gold_std_patients_$(run_dataset_name).csv")
    report_path = joinpath(output_dir, "gold_std_filter_report_$(run_dataset_name).csv")

    CSV.write(ids_path, DataFrame(
        patient=[patient.id for patient in gold_patients],
        dataset=gold_dataset,
    ))
    CSV.write(report_path, report)

    return (
        patients=gold_patients,
        ids_path=ids_path,
