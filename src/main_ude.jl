...
nn_depth = 2;
nn_width = 8;
# inputs_str = input_dim == 2 ? "τ" : "τ, Cs0, Cc0";
inputs_str = "τ";
if input_dim == 3
    inputs_str = "Cs0, Cc0, τ";
elseif input_dim == 5
    inputs_str = "τ, a, b, Cs0, Cc0";
end
...
# models = [];
# # models = [ctntUDEModel(p.ode[5*(j-1) + 1:5*j], chain, (training_dataset[j].timepoints[1], training_dataset[j].timepoints[end])) for j in eachindex(training_dataset)];
# for p in initial_parameters # p = initial_parameters[k]
#     models_array = [ctntUDEModel(p.ode[5*(j-1) + 1:5*j], chain, (0.0, training_dataset[j].timepoints[end])) for j in eachindex(training_dataset)];
...
local_models = [
    ctntUDEModel(θ_dummy.ode[N_params*(j-1)+1:N_params*j], chain,
...
for k in 1:n_optsol
    ...
    # try
        ...
        for (i, model) in enumerate(models_valid)
            ...
            # opt_model = ctntUDEModel(optsol_lbfgs.u, chain, tspan);
