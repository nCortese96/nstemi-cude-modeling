...
function plot_wins_bar(candidate_ranking::DataFrame; output_path::AbstractString)
    labels = string.(candidate_ranking.candidate_id)
    p = bar(
        labels,
        candidate_ranking.wins_smape,
...
function plot_median_smape_vs_wins(candidate_ranking::DataFrame; output_path::AbstractString)
    p = Plots.scatter(
        candidate_ranking.median_smape,
        candidate_ranking.wins_smape,
        xlabel = "Median validation sMAPE",
        ylabel = "sMAPE wins",
        title = "Candidate ranking: wins vs median sMAPE",
        legend = false,
        size = (900, 700),
    )

    for i in 1:nrow(candidate_ranking)
        annotate!(p,
            candidate_ranking.median_smape[i],
            candidate_ranking.wins_smape[i],
            Plots.text(string(candidate_ranking.candidate_id[i]), 8, :left)
        )
    end
...
function plot_pairwise_heatmap(candidate_ids::Vector{String}, pairwise_wins::AbstractMatrix; output_path::AbstractString)
    p = Plots.heatmap(
        candidate_ids,
        candidate_ids,
        pairwise_wins,
        ...
        title = "Pairwise head-to-head sMAPE wins",
        xrotation = 45,
        size = (1000, 850),
    )
    savefig(p, output_path)
...
function plot_topk_ecdf_smape(candidate_ranking::DataFrame, candidate_ids::Vector{String}, smape_mat::AbstractMatrix;
    ...
    for id in ordered_ids
        ...
        Plots.plot!(p, xs, ys, linewidth = 2, label = id)
    end
...
function plot_best_vs_runnerup(candidate_ranking::DataFrame, candidate_ids::Vector{String}, smape_mat::AbstractMatrix;
    ...
    x = vec(smape_mat[:, id_to_col[best_id]])
    y = vec(smape_mat[:, id_to_col[runner_id]])
    ...
    p = Plots.scatter(
        x,
        y,
        xlabel = "$(best_id) sMAPE",
        ylabel = "$(runner_id) sMAPE",
        title = "Paired validation comparison: best vs runner-up",
        label = false,
        size = (800, 800),
    )
    Plots.plot!(p, [lo, hi], [lo, hi], linewidth = 2, label = "y = x")
    savefig(p, output_path)
    return p
end
...
function run_validation_model_selection(csv_paths::Vector{String};
    ...
    plot_pairwise_heatmap(mats.candidate_ids, pairwise_wins; output_path = joinpath(output_dir, "pairwise_smape_wins_heatmap.png"))
