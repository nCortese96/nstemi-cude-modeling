...
bars. The `plots` mode currently reads existing residual CSVs and regenerates
only residual figures, without recomputing residual tables.
"""
...
length(ARGS) <= 1 || error("Model diagnostics accepts at most one mode argument. Valid modes: all, plots.")
diagnostic_mode in ("all", "plots") ||
    error("Invalid model diagnostics mode '$(diagnostic_mode)'. Valid modes: all, plots.")
